...
//static void MD5Print(unsigned char *);
...
string MDString(const char * str)
{
    /*MD5_CTX context;
    ...
    MD5_CTX context;
    MD5Init(&context);
    long long dwoffset = 0;
    while(dwoffset < len){
        long long partlen  = len - dwoffset;
        UINT4 dwlen = -1;
        if(partlen < dwlen){
            dwlen = (UINT4)partlen;
        }
        MD5Update(&context,(unsigned char *)str+dwoffset,(int)dwlen);
        dwoffset += dwlen;
    }
    unsigned char digest[16];
    MD5Final(digest, &context);
    ...
    int count  =0;
    for(int i = 0;i<16;i++){
        count += snprintf(output + count, 33-count, "%02x",digest[i]);
    }
    string res = string(output,32);
    return res;
}
